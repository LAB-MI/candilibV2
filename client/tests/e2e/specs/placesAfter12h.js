...
describe('Display new place after 12h', () => {
  ...
  before(() => {
    ...
    cy.updatePlaces({}, { visibleAt: now.set({ hour: 12, minute: 0, second: 0, millisecond: 0 }).toUTC() })
    cy.updatePlaces({ date: { $gte: dateAt1Months.toUTC() } }, { visibleAt: now.set({ hour: 12, minute: 0, second: 0, millisecond: 0 }).plus({ days: 1 }).toUTC() })
    cy.updatePlaces({ date: { $gte: dateAt2Months.toUTC() } }, { visibleAt: now.set({ hour: 12, minute: 0, second: 0, millisecond: 0 }).toUTC() })
    cy.updatePlaces({ date: { $gte: dateAt3Months.toUTC() } }, { visibleAt: now.set({ hour: 12, minute: 0, second: 0, millisecond: 0 }).plus({ days: 1 }).toUTC() })
