...
docker run -it -v $(pwd)/${SETUP_DIR}:/app/dev-setup -v $(pwd)/${SRC_DIR}:/app/src -e MONGO_URL=$MONGO_URL --network ${candilib_api_network} --rm --link candilib_api:api_init --entrypoint "" ${candilib_api_image}  /bin/bash -c '( cd /app && export MONGO_URL && npm run dev-setup ) '
# recreation index
