...
PlaceSchema.pre('save', async function preSave () {
  ...
  const places = await model.find({inspecteur: place.inspecteur, date: {
    $gte: getDateTimeFrFromJSDate(place.date).startOf('day').toJSDate(),
    $lt: getDateTimeFrFromJSDate(place.date).endOf('day').toJSDate(),
  }}).populate('inspecteur')
  console.log('Test', places.length)
  if (places.length) {
    throw new Error(`Incohérence dans le planning: l'inspecteur ${inspecteur.nom}`)
  }
