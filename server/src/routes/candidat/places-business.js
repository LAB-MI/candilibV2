...
const getDateETGExpiered = async candidatId => {
  const { dateReussiteETG } = await findCandidatById(candidatId, {
    _id: 0,
    dateReussiteETG: 1,
  })
  const luxonDateETGExpired = getFrenchLuxonFromJSDate(dateReussiteETG).plus({
...
export const getDatesByCentreId = async (
  ...
  const luxonDateETGExpired = await getDateETGExpiered(candidatId)
